...
def init(
    ...
    path: Optional[Path] = typer.Option(None, "--path", "-p", help="Path where to create the project"),
    project_type: Optional[str] = typer.Option(None, "--type", "-t", help="Project type (rust, python, chrome-extension, etc.)"),
    template: Optional[str] = typer.Option(None, "--template", help="Specific template to use"),
...
def create_template(
    ...
    source: Optional[Path] = typer.Option(None, "--source", "-s", help="Source directory to create template from"),
) -> None:
...
def _interactive_project_type(current_type: Optional[str]) -> Optional[str]:
...
def _interactive_template(current_template: Optional[str]) -> Optional[str]:
...
def _show_next_steps(project_name: str, project_path: Path, project_type: Optional[ProjectType]) -> None:
