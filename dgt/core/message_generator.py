...
class MessageGenerator:
    ...
    def generate_smart_message(
        self, 
        files: List[str], 
        line_numbers: Optional[Dict[str, str]] = None,
        use_llm: bool = False
    ) -> str:
    ...
    def _generate_llm_message(self, files: List[str], categorized: Dict[str, List[str]]) -> Optional[str]:
...
Generate only the commit message, no explanation:"""
    ...
    def _detect_project_type(self) -> str:
        ...
        if (project_root / "Cargo.toml").exists():
            return "Rust"
        elif (project_root / "manifest.json").exists():
